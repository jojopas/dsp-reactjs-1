...
export default function PDPTop({type, pdp, currVideo = null}) {
    ...
    const measureDescRef = React.useCallback(node => {
        ...
        if (node !== null) {
            const maxLines = 3;
            const lineHeight = parseInt(getComputedStyle(node, null).getPropertyValue("line-height"));
    ...
    const launchPlayer = (vId, currVideo) => {
        if(vId) {
            (currVideo && vId !== currVideo) ? store.loadVideo({id: vId}): null;
        ...
        <div className="pdp-top">
            ...
            <div className="pdp-top-info">
                ...
                <div className="pdp-top-ratingBar">
                    ...
                    { type === 'movie' ? (
                        <span className="pdp-top-duration">{duration(pdp.duration, true)}</span>
                    ) : pdp.seasons ? (
                        <span className="pdp-top-seasons">{`${pdp.seasons.length} ${constants.SEASON}${pdp.seasons.length > 1 ? 's' : ''}`}</span>
                    ) : null}
                </div>
                <div className="pdp-top-buttonBar">
                    <Button className="pdp-top-watchNowButton button-current" inner={constants.WATCH_NOW} onClick={() => launchPlayer((pdp.videoId) ? pdp.videoId : pdp.seasons[0].cards[0].id, currVideo)} />
                    {/*<Button className="pdp-top-startOverButton" inner={constants.START_OVER} />*/}
                    { pdp.trailer ? (
                        <Button className="pdp-top-trailerButton" inner={constants.TRAILER} onClick={() => launchPlayer(pdp.trailer)} />
                    ) : null}
                </div>
                { store.isBreakpoint ? (
                    <>
                        <div className="pdp-top-toolBar">
                            ...
                            <span className="pdp-top-shareButton" title={constants.SHARE} onClick={triggerShare}><InlineSVG type="share"/></span>
                        </div>
                        <div className={`pdp-top-description${descIsClamped ? ' pdp-top-description-more' : ''}`} ref={measureDescRef}>{pdp.description}</div>
                        {descHasClamp ? (
                            <Button
                                className="button-small pdp-top-description-moreButton"
                                inner={descIsClamped ? constants.MORE : constants.LESS}
                                onClick={toggleDesc}
                            />
                        ) : null }
                    </>
                    ...
                    <>
                        ...
                        <div className="pdp-top-toolBar">
                            ...
                            <span className="pdp-top-dislikeButton">Start Over</span>
                            <span className="pdp-top-watchlistButton">Trailer</span>*/}
                            <span className="pdp-top-shareButton" title={constants.SHARE} onClick={triggerShare} onMouseLeave={() => setShareTipOpen(false)}>
                                <InlineSVG type="share"/>
                                <Share className={shareTipOpen ? 'share-tip-open' : ''} title={pdp.title} />
                            </span>
                        </div>
                    </>
                    )}
