...
export default function Player({pageType, video = null, autostart = true, showPlayer}) {
    ...
    React.useEffect(() => {
        (async () => {
            ...
            if (!store.isPlayerSetup && video) {
                ...
                const videoData = (video && video.id) ? {id: video.id} : (video && video.url) ? {url: video.url, vmap: video.vmap, meta: video.meta} : null;
                const player = await setupPlayer(plyrRef, videoData, {autostart, ssid: store.springServeId, vodPage});
    ...
    React.useEffect(() => {
        const plyrStyle = lightBoxPageTypes.includes(pageType) ? 'lightbox' : 'inline';
        ...
        if (plyrStyle === 'lightbox') {
            if (showPlayer) {
                ...
                if(
                    store.playerInstance && store.playerInstance.vjs &&
                    (!store.playerInstance.vjs.autoplay() || (store.playerInstance.vjs.paused() && store.playerInstance.vjs.currentTime() > 1))
                ){
            ...
            if(window){
                ...
                showPlayer ? body.classList.add('bodyLocked') : body.classList.remove('bodyLocked');
            }
        } else {
        ...
        return () => {
            const body = document.querySelector('body');
            body.classList.remove('bodyLocked');
        }
    ...
    const playEvent = () => {
        ...
        if (playerCloseButton && playerCloseButton.current && !store.playerInstance.vjs.userActive()) {
            fadeOut(playerCloseButton.current);
        }
    ...
    const adsReady = () => {
        store.playerInstance.vjs.ima.addEventListener(google.ima.AdEvent.Type.STARTED, adsStarted);
        store.playerInstance.vjs.ima.addEventListener(google.ima.AdEvent.Type.COMPLETE, adsCompleted);
    }
    ...
    React.useEffect(() => {
        ...
        return () => {
            ...
            if (store.playerInstance && store.playerInstance.vjs) {
                ...
                if(store.playerInstance.vjs.ima && store.playerInstance.vjs.ima.removeEventListener) {
                    store.playerInstance.vjs.ima.removeEventListener(google.ima.AdEvent.Type.STARTED, adsStarted);
                    store.playerInstance.vjs.ima.removeEventListener(google.ima.AdEvent.Type.COMPLETE, adsCompleted);
                }
            }
    ...
    return useObserver(() => {
        return (
        <div className={`player-container player-${playerStyle}`} style={{display: 'none'}} ref={outerPlyrRef}>
            {lightBoxPageTypes.includes(pageType) ? (<button ref={playerCloseButton} className="player-close" onClick={() => closeLightBox()} title="Close"><InlineSVG type="close"/></button>) : null}
