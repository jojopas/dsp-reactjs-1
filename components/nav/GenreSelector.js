...
export default function GenreSelector({className, style, links, type, changeListener, hoveredListener}) {
    ...
    const handleChange = (e) => {
        ...
        if(type === 'live') {
            ...
            changeListener(e.target.value === 'All Genres' ? '' : e.target.value);
            setIsHovered(false);
        } else {
            let url = `/${type}`;
    ...
    return (
        <span className={className ? `genreSelector ${className}` : 'genreSelector'} style={style}>
            <select
                onChange={handleChange}
                value={type === 'live' ? currentGenre || '' : currentGenre || `/${type}`}
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
            >
                <option key='all-genres' value={type === 'live' ? '' : `/${type}`}>
                    {type === 'live' ? 'All Channels' : 'All Genres'}
                </option>
                {links.map((link) => (
                    <option key={link.id} value={link.as}>{link.inner}</option>
                ))}
            </select>
            <span className={`genreSelector-selected${isHovered ? ' genreSelector-hover' : ''}`}>
                {currentName}
                <span className="genreSelector-arrow"><InlineSVG type="arrow"/></span>
            </span>
        </span>
    )
}
