...
export default function Channel({session, config, slug, page, pageError, seoObj, pageType}) {
    ...
    const { data, error, isValidating } = useSWR('/api/dsp/live/epg', async (url) => await fetchData(url), { refreshInterval: constants.EPG_POLLING*1000});
        ...
        if (data && data.data && data.data.data && data.data.data.channels) {
            ...
            setResult((prevState) => {
                return prevState.map((prevCh) => {
                   ...
                   if (ch && prevCh && prevCh.program && prevCh.program.length > 1 && (
    ...
    React.useEffect(() => {
        ...
        if (!currentSlug) {
            ...
            if (!store.currentChannel || !doesChannelExist(store.currentChannel)) {
                ...
                if (firstChannelId) {
                    ...
                    (isFirstVideoRef.current) ? setFirstVideo({id: currentChannelId}) : store.loadVideo({id: currentChannelId});
            ...
            (isFirstVideoRef.current) ? setFirstVideo({id: getVideoIdFromSlug(currentSlug)}) : store.loadVideo({id: getVideoIdFromSlug(currentSlug)});
    ...
    React.useEffect(() => {
        if(document) {
            document.addEventListener(visibilityChange, visibilityUpdate, false);
        }

        return () => {
            if(document) {
                //console.log('visibility listener removed');
                document.removeEventListener(visibilityChange, visibilityUpdate, false);
            }
        }
    }, []);
    ...
    const activatePlayerUI = (show = true) => {
        ...
        if(!shouldShow && (isScrollingRef.current || genreIsHoveredRef.current || genreDelayRef.current)){
            shouldShow = true;
        }
    ...
    let pDownHandler, pMoveHandler, pLeaveHandler, mMoveHandler, mLeaveHandler,
        containerTouch, containerTouchEnd, inactiveTimer, inactiveFix;
    const hoverHandler = () => {
        ...
        containerTouchEnd = (e) => {
            clearTimeout(inactiveTimer);
            if(!(e.target instanceof HTMLSelectElement)) {
                inactiveTimer = setTimeout(()=> activatePlayerUI(false), constants.EPG_UI_TIMEOUT*1000);
            }
        };
        ...
        if (!isMobile) {
            ...
            if (hasPointerEvents()) {
                ...
                containerEl.addEventListener('pointerdown', pDownHandler, false);
                pMoveHandler = (e) => {
                    if (getPointerType(e) === 'mouse') {
                        containerMouse(e);
                    }
                }
                containerEl.addEventListener('pointermove', pMoveHandler, false);
                pLeaveHandler = (e) => {
                    // Windows Touch (Use touchstart logic)
                    if (getPointerType(e) !== 'touch') {
                        activatePlayerUI(false);
                ...
                containerEl.addEventListener('pointerleave', pLeaveHandler, false);
            } else {
                mMoveHandler = (e) => {
                    containerMouse(e);
                }
                ...
                containerEl.addEventListener('mouseleave', mLeaveHandler, false);
            }
        } else {
